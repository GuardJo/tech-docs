...
Java에서는 하나의 파일에 여러 톱클래스 파일을 아래와 같이 구현해도 컴파일이나 런타임 시에 별다른 오류가 발생하지 않는다.

```java
// Test.java
class Test1 {
	final static String TEST = "Test1";
}
class Test2 {
	final static String TEST = "Test2";
}
```

예를 들어 위와 같이 톱레벨 클래스 두 개가 선언된 파일이 존재할 때, 아래와 같이 동일한 톱클래스명을 지닌 파일이 존재할 수 있다.

```java
// Mock.java
class Test1 {
	final static String TEST = "Mock1";
}
class Test2 {
	final static String TEST = "Mock2";
}
```

이럴 경우 java 컴파일러가 어떤 파일을 우선적으로 컴파일 하냐에 따라, 반환되는 값이 완전히 달라질 수 있다.

이러한 문제점이 있기에 보통은 한 파일 내에 톱레벨의 클래스는 하나만 선언하는 것이, 개발하는 입장과 사용하는 입장에서도 문제의 소지를 줄일 수 있다.