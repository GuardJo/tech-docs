...
이러한 `Generic Method`의 경우, **타입 매개변수** 를 정의하는 과정이 필요하며, 실제 런타임 환경에서 호출 시에는 아래와 같은 과정을 내부적으로 수행하게 된다.
...
- *그렇기에 일반적으로 `Generic Class`를 매개변수로 하는 메소드의 경우 `Generic Class`보다 **Wild Card**를 사용하는것을 권장함*
...
```
위와 같이 `Generic Method`의 경우, 런타임 시점에 **타입 인자**가 매핑되어, 반환 타입으로 **타입 인자**로 주었던 데이터 타입을 그대로 반환할 수 있지만, **Wild Card**의 경우 컴파일 시점에 이미 메소드에 지정된 타입만을 반환 받을 수 있게 된다.
